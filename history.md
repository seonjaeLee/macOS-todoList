...
- 항상 위에 노출(플로팅)하는 열기구(기구) 아이콘 기능 추가:
  - 위젯 헤더 접기/닫기 왼쪽에 세련된 미니 열기구 SVG 아이콘 버튼(`btn-float`) 추가.
  - 클릭 시 둥둥 떠 있는 플로팅 상태를 시각화하는 호버 모션(translateY) 및 active 상태 스타일(채워진 색상) 구현.
  - 저명도(`low-light`) 대비 개선 모드에서 열기구 아이콘 색상이 흰색 계열로 자동 반전 및 채워지도록 보강.
  - `BrowserWindow.setAlwaysOnTop(true, 'floating')` 방식을 채택하여 다른 프로그램 위에서도 자유롭게 드래그 및 리사이즈가 가능하도록 구현.
  - 플로팅 상태를 로컬 저장소(`widgets.json`)에 저장하여 앱 종료 후에도 상태를 완벽히 영속 복원.
- 메모 목록 팝업창 연동 및 실시간 UI 상호 동기화:
  - 메모 목록 리스트의 각 아이템 액션 영역에 작은 풍선 아이콘 추가.
  - 목록 내 풍선을 누르면 해당 위젯의 항상 위 노출 상태가 동기화되어 토글되도록 처리.
  - 위젯 스스로 토글되거나 외부 메모 목록에서 토글되었을 때, 메인과 렌더러 간 실시간 UI 동기화(IPC `external-always-on-top-update`) 구조 완비.
